   ...
   <div className="flex flex-col md:flex-row md:h-[600px] min-h-screen bg-white rounded-none md:rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
     ...
     {showHistory && (
       <div className="fixed inset-0 z-50 flex md:hidden">
         {/* Overlay */}
         <div className="absolute inset-0 bg-black/50 backdrop-blur-sm transition-opacity duration-300 opacity-100 pointer-events-auto" onClick={() => setShowHistory(false)}></div>
         {/* Drawer */}
         <div className="relative h-full bg-gray-900 text-white flex flex-col shadow-2xl transition-transform duration-300 ease-in-out w-4/5 max-w-xs translate-x-0 rounded-r-2xl p-4 drop-shadow-xl">
           <button
             className="absolute top-4 right-4 z-30 p-2 rounded-lg bg-gray-800 hover:bg-gray-700 text-white shadow transition-colors"
           ...
           {/* Sidebar content as before, but always expanded on mobile */}
           <div className="flex-1 flex flex-col h-full">
             {/* New chat button */}
             <button
               onClick={createNewConversation}
               className="flex items-center gap-2 px-3 py-2 rounded-lg text-white hover:bg-gray-800 transition-colors text-base font-medium mt-4"
               aria-label="New chat"
               title="New chat"
             >
               <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
             ...
             <span className="font-semibold text-lg mt-2 mb-1 text-gray-400 px-4">Chats</span>
             <div className="flex-1 overflow-y-auto px-2 pb-2 w-full" style={{ minHeight: 0 }}>
               <style>{`
                 .custom-scrollbar::-webkit-scrollbar {
                   width: 8px;
                   background: #1a202c;
